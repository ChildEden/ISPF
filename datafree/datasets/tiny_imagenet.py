...
class TinyImageNet(torch.utils.data.Dataset):
    def __init__(self, root, split='train', transform=None):
        ...
        self.classes = [k for k, v in self.wnid2index.items()]
        ...
        self.targets = np.array([int(target) for _, target in self.data])
        self.data = np.array([data for data, _ in self.data])
