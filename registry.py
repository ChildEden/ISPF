...
MODEL_DICT = {
    ...
    'allcnn': classifiers.AllCNN.allcnn,
}
...
class SVHN(datasets.SVHN):
    ...
    def __getitem__(self, index):
        img, label = super(SVHN, self).__getitem__(index)
...
def get_dataset(name: str, data_root: str='data', return_transform=False, split=['A', 'B', 'C', 'D']):
    ...
    if name=='mnist':
        ...
        data_root = os.path.join( data_root, 'CIFAR' ) 
        ...
        train_dst = SVHN(data_root, split='train', download=True, transform=train_transform)
        val_dst = SVHN(data_root, split='test', download=True, transform=val_transform)
